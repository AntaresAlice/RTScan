...
std::vector<std::pair<uint32_t, uint32_t>> bench_versioned_insert_range(
  ...
  for (std::size_t exp = 0; exp < num_experiments; exp++) {
    ...
    double mu = tree.compute_memory_usage();
    std::cout << "Memory usage: " << mu << std::endl;
...
int main(int argc, char **argv) {
  ...
  uint32_t num_range_query = 1; // for test by wzm
  ...
  h_range_upper[0] = 0.5 * span;
  std::cout << "selectivity: " << 0.5 << ", range: " << h_range_upper[0] << std::endl;
