...
bench_rates bench_versioned_insert_range(thrust::device_vector<KeyT> &d_keys,
  ...
  for (std::size_t exp = 0; exp < num_experiments; exp++) {
    ...
    std::cerr << "Start to build tree" << std::endl;
    BTree tree;
  ...
  // auto h_range_results = thrust::host_vector<PairT>(d_results);
  // for (int i = 0; i < 50; i++) {
  //   // if (h_range_results[i].first != invalid_key) {
  //     std::cout << "[key] " << h_range_results[i].first << ", [value] " << h_range_results[i].second << std::endl;
  //   // }
  // }
  ...
  return {insertion_rate, ops_rate, average_insertion_seconds * 1000, average_ops_seconds * 1000};
}

void getDataFromFile(const char *DATA_PATH, uint32_t *initial_data, int bindex_num, int N) {
  FILE* fp;
  ...
  for (int bindex_id = 0; bindex_id < bindex_num; bindex_id++) {
    if (fread(initial_data, sizeof(uint32_t), N, fp) == 0) {
      printf("init_data_from_file: fread faild.\n");
      exit(-1);
    }
    printf("[CHECK] col %d  first num: %u  last num: %u\n", bindex_id, initial_data[0], initial_data[N - 1]);
  }
  fclose(fp);
}
...
int main(int argc, char **argv) {
  ...
  uint32_t selectivity          = get_arg_value<uint32_t>(arguments, "selectivity").value_or(1);
  ...
  std::string input_file = get_arg_value<std::string>(arguments, "input-file").value_or("./");
  ...
  size_t avail_curr_gpu_mem, used;
  ...
  auto h_range_lower = std::vector<key_type>(num_range_query);
  auto h_range_upper = std::vector<key_type>(num_range_query);
  getDataFromFile(input_file.c_str(), 
                  h_keys.data(), 
                  1, 
                  initial_tree_size);

  // * For selectivity (0, 1, 0.1)
  uint32_t span = invalid_key / 10;
  h_range_upper[0] = selectivity * span;
  std::cout << "selectivity: " << selectivity << ", range: " << h_range_upper[0] << std::endl;

  // * For Skew 
  // h_range_upper[0] = selectivity;
  // std::cout << "selectivity: " << selectivity << ", range: " << h_range_upper[0] << std::endl;
  ...
  std::cerr << "Get input data" << std::endl;
  ...
  std::experimental::filesystem::create_directories(report_dir);
  ...
  bool output_file_exist = std::experimental::filesystem::exists(report_dir + filename);
  ...
  if (!output_file_exist) {
    ...
    // * comments below codes to test the occupied GPU memory
    // result_output << "vblink_slab_out_of_place_insert" << ',';
    // result_output << "vblink_slab_out_of_place_concurrent_ops" << ',';
    // result_output << "vblink_slab_out_of_place_insert_time" << ',';
    // result_output << "vblink_slab_out_of_place_concurrent_ops_time" << ',';

    result_output << "blink_slab_insert" << ',';
    result_output << "blink_slab_concurrent_ops" << ',';
    result_output << "blink_slab_insert_time" << ',';
    result_output << "blink_slab_concurrent_ops_time" << ',';
    result_output << "blink_base_data_gpu_mem" << ",";
    result_output << "blink_gpu_memory" << ",";
    result_output << '\n';
  }
  ...
  //   result_output << rates.insertion_time << ',';
  //   result_output << rates.ops_time << ',';
  // }

  {
    auto rates = bench_versioned_insert_range<blink_tree_slab_type, false>(d_keys, // supportsVersioning = false
                                                                           d_values,
                                                                           initial_tree_size,
                                                                           d_range_lower,
                                                                           d_range_upper,
  ...
  used = avail_init_gpu_mem - avail_curr_gpu_mem;
  // cout << "[Mem] Host Optix context used(MB): " << 1.0 * used / (1 << 20) << endl;
  result_output << base_data_gpu_mem << ",";
  result_output << 1.0 * used / (1 << 20) << ",";
  result_output << '\n';
}
