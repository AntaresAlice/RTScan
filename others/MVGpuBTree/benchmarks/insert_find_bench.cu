...
bench_rates bench_versioned_insertion_find(thrust::device_vector<KeyT>& d_keys,
  ...
  for (std::size_t exp = 0; exp < num_experiments; exp++) {
    ...
    if constexpr (supportsVersioning) {
      std::cout << "supportsVersioning=" << supportsVersioning << ", in_place=" << in_place << std::endl;
  ...
  return {insertion_rate, find_rate, average_insertion_seconds * 1000, average_find_seconds * 1000};
}

void getDataFromFile(char *DATA_PATH, uint32_t *initial_data, int bindex_num, int N) {
  FILE* fp;
...
int main(int argc, char** argv) {
  ...
  uint32_t num_queries = get_arg_value<uint32_t>(arguments, "num-queries").value_or(1'000'000);
  ...
  auto d_results   = thrust::device_vector<value_type>(num_queries, invalid_value);
  ...
  auto h_find_keys = std::vector<value_type>(num_queries);
  getDataFromFile("/home/wzm/bindex-raytracing/data/uniform_data_1e8_3.dat", 
                  h_keys.data(), 
                  1, 
                  num_keys);
  // * For batch query
  uint32_t span = invalid_key / (num_queries + 1);
  for (uint32_t i = 0; i < num_queries; i++) {
    h_find_keys[i] = (i + 1) * span;
  }
