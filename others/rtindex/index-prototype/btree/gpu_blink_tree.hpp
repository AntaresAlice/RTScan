...
struct gpu_blink_tree {
  ...
  // EDIT jh: separated allocation and bulk-load
...
// MODIFICATION jh: upper bound is now inclusive, added aggregation
  template <typename tile_type, typename DeviceAllocator>
  DEVICE_QUALIFIER value_type modified_cooperative_range_query(const Key& lower_bound,
                                                              const Key& upper_bound,
                                                              const tile_type& tile,
                                                              DeviceAllocator& allocator,
                                                              ...
                                                              bool concurrent = false)
  {
    ...
    while (true) {
      ...
      if (is_leaf) {
        bool keep_traversing = true;
        do {
          sum += current_node.modified_get_in_range(lower_bound, upper_bound, stored_values);
          keep_traversing = upper_bound >= current_node.get_high_key();
          if (keep_traversing) {
            current_node_index = current_node.get_sibling_index();
            current_node       = node_type(
...
// MODIFICATION jh: this is required to write custom kernels
public:
