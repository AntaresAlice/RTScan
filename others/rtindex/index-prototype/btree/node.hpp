...
struct btree_node {
  ...
  DEVICE_QUALIFIER value_type modified_get_in_range(const key_type& lower_bound,
                                                    const key_type& upper_bound,
                                                    const value_type* stored_values) const {
    bool is_valid_lane   = tile_.thread_rank() != metadata_lane_;
    bool in_range        = lane_pair_.first >= lower_bound && lane_pair_.first <= upper_bound;
    auto in_range_ballot = tile_.ballot(in_range && is_valid_lane);
    auto first_lane      = __ffs(in_range_ballot) - 1;
    
    // MODIFIED jh: warp aggregation
    value_type value = is_valid_lane && in_range ? stored_values[lane_pair_.second] : 0;
    value_type value_sum = 0;
    for (auto i = 0; i < metadata_lane_; i++) {
      value_sum += tile_.shfl(value, i);
    }
    return value_sum;
  }
  template <typename T>
