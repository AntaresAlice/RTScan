...
void refine_with_GPU(BinDex **bindexs, BITS *dev_bitmap, const int bindex_num)
{
  ...
  while (scan_refine_in_position != bindex_num && !scan_skip_refine)
  {
  }
...
void exp_opt(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "khl:r:f:a:b:w:m:s:t:p:q:g:v:c:d:z:e:")) != -1) {
    switch (opt) {
      case 'h':
        printf(
            "Usage: %s \n"
  ...
  cudaMemset(dev_bitmap[bindex_num], 0, bitmap_len * sizeof(BITS)); // TODO: change bitmap_len here to a max len
