...
void raw_scan_entry(std::vector<CODE>* target_l, std::vector<CODE>* target_r, CODE target1, CODE target2, std::string search_cmd, BinDex* bindex, BITS* bitmap, BITS* mergeBitmap, CODE* raw_data) {
  ...
  // std:lock_guard<mutex> lock(bitmapMutex);
...
void scan_multithread(std::vector<CODE> *target_l, std::vector<CODE> *target_r, CODE target1, CODE target2, std::string search_cmd, BinDex *bindex, BITS *bitmap, BITS *mergeBitmap)
{
    ...
    // std:lock_guard<mutex> lock(bitmapMutex); // TODO: is this necessary?
    if (mergeBitmap != bitmap) {
      std::thread threads[THREAD_NUM];
      int start_idx = 0;
      int end_idx = 0;
...
void exp_opt(int argc, char *argv[]) {
  ...
  while ((opt = getopt(argc, argv, "kha:l:r:o:f:n:b:w:m:s:p:")) != -1) {
    switch (opt) {
      case 'h':
        printf(
            "Usage: %s \n"
