#include <optix.h>

#include "optixScan.h"

#include <sutil/vec_math.h>

extern "C" {
__constant__ Params params;
}

static __forceinline__ __device__ void computeRay( uint3 idx, uint3 dim, float3& origin, float3& direction ) {
    if (params.column_num == 3) {
        origin = {
            float(params.predicate[0] + idx.z * params.ray_stride),
            float(params.predicate[2] + (idx.x + 0.5) * params.aabb_width),
            float(params.predicate[4] + (idx.y + 0.5) * params.aabb_width)
        };
    } else if (params.column_num == 2) {
        origin = {
            0.0f,
            float(params.predicate[0] + (idx.x + 0.5) * params.aabb_width),
            float(params.predicate[2] + (idx.y + 0.5) * params.aabb_width)
        };
    }
    direction = make_float3(1, 0, 0);
}

static __forceinline__ __device__ void set_result(unsigned int *result, int idx) {
    int size = sizeof(unsigned int) << 3;
    int pos = idx / size;
    int pos_in_size = idx & (size - 1);
    if (params.inverse) {
        atomicAnd( result + pos, ~(1 << (size - 1 - pos_in_size)) );
    } else {
        atomicOr( result + pos, 1 << (size - 1 - pos_in_size) );
    }
}

extern "C" __global__ void __raygen__rg() {
    // Lookup our location within the launch grid
    const uint3 idx = optixGetLaunchIndex();    
    const uint3 dim = optixGetLaunchDimensions(); 

    // Map our launch idx to a screen location and create a ray from the camera
    // location through the screen 
    float3 ray_origin, ray_direction;
    computeRay( idx, dim, ray_origin, ray_direction );

    // Trace the ray against our scene hierarchy
    unsigned int intersection_test_num = 0;
    unsigned int hit_num = 0;
    optixTrace(
        params.handle,
        ray_origin,
        ray_direction,
        0.0f,                               // Min intersection distance
        float(params.ray_stride),           // Max intersection distance
        0.0f,                               // rayTime -- used for motion blur
        OptixVisibilityMask( 255 ),         // Specify always visible
        OPTIX_RAY_FLAG_NONE,
        0,                                  // SBT offset   -- See SBT discussion
        1,                                  // SBT stride   -- See SBT discussion
        0,                                  // missSBTIndex -- See SBT discussion
        intersection_test_num,
        hit_num
    );
#if DEBUG_ISHIT_CMP_RAY == 1
    atomicAdd(params.intersection_test_num, intersection_test_num);
    atomicAdd(params.hit_num, hit_num);
    params.ray_primitive_hits[dim.x * dim.y * idx.z + dim.x * idx.y + idx.x] = hit_num;
#endif
}

extern "C" __global__ void __miss__ms() {
}

extern "C" __global__ void __intersection__cube() {
    unsigned int primIdx = optixGetPrimitiveIndex();
    double2 point = params.points2[primIdx];

#if DEBUG_ISHIT_CMP_RAY == 1
    optixSetPayload_0(optixGetPayload_0() + 1);
#endif
    if (point.x > params.predicate[0] && point.x < params.predicate[1] &&
        point.y > params.predicate[2] && point.y < params.predicate[3] ) {
        set_result(params.result, primIdx);
#if DEBUG_ISHIT_CMP_RAY == 1
        optixSetPayload_1(optixGetPayload_1() + 1);
#endif
    }
}

// anyhit maybe called repeatedly.
extern "C" __global__ void __anyhit__triangle() {
    HitGroupData* hit_group_data = reinterpret_cast<HitGroupData*> (optixGetSbtDataPointer());
    unsigned int primIdx = optixGetPrimitiveIndex();
    double3 point = params.points3[(primIdx << 1) + primIdx];
    
    unsigned int intersection_test_num = optixGetPayload_0();
    optixSetPayload_0(intersection_test_num + 1);
    if (point.x > hit_group_data->x1 && point.x < hit_group_data->x2 &&
        point.y > hit_group_data->y1 && point.y < hit_group_data->y2 && 
        point.z > hit_group_data->z1 && point.z < hit_group_data->z2 ) {
        set_result(params.result, primIdx);
    }
    optixIgnoreIntersection();
}
